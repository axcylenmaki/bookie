...
$produk = mysqli_fetch_assoc(
  mysqli_query($conn,"SELECT * FROM produk WHERE id='$id' AND penjual_id='$idPenjual'")
);
...
if (isset($_POST['simpan'])) {
  ...
  if (!empty($_FILES['gambar']['name'])) {
    ...
    if (in_array($ext, $allowed_ext)) {
      if ($_FILES['gambar']['size'] <= 2 * 1024 * 1024) { // 2MB max
        $gambarBaru = "produk_".time()."_".rand(100,999).".".$ext;
  ...
  if (!isset($error)) {
    mysqli_query($conn,"UPDATE produk SET 
      ...
      modal='$modal',
      margin='$margin',
      keuntungan='$keuntungan_per_unit',
      deskripsi='$deskripsi'
      $updateGambar
      WHERE id='$id' AND penjual_id='$idPenjual'
    ");
...
<style>
/* CSS khusus untuk halaman edit produk */
.form-container {
  max-width: 900px;
  margin: 0 auto;
  background: white;
...
<div>
  ...
  <p class="text-muted mb-0">Perbarui informasi produk buku</p>
</div>
...
<div class="row g-4">
    ...
    <div class="row g-3">
      <div class="col-md-12">
        ...
        <input type="text" name="nama" class="form-control" required 
               value="<?= htmlspecialchars($produk['nama_buku']) ?>" maxlength="150">
      ...
      <div class="col-md-6">
        ...
        <input type="text" name="penulis" class="form-control" 
               value="<?= htmlspecialchars($produk['penulis'] ?? '') ?>" 
      ...
      <div class="col-md-6">
        ...
        <div class="input-group">
          <input type="number" name="stok" class="form-control" min="0" required 
                 value="<?= $produk['stok'] ?>" id="stokInput">
    ...
    <div class="row g-3">
      ...
      <div class="col-md-12">
        <div class="image-comparison">
          <div class="image-box">
            ...
            <small class="text-muted d-block mt-2"><?= htmlspecialchars($produk['gambar']) ?></small>
          </div>
          
          <div class="image-box">
            <h6>Pratinjau Gambar Baru</h6>
            <div id="imagePreview" class="d-none">
              <img id="preview" class="preview-image new-image"
                   style="max-width: 100%; height: auto;">
            </div>
      ...
      <div class="col-md-12">
        ...
        <small class="text-muted d-block mt-1">
          Kosongkan jika tidak ingin mengganti gambar. Format: JPG, PNG, GIF, WebP | Maksimal: 2MB
        </small>
      </div>
    </div>
  </div>
        ...
        <div class="mb-4">
          ...
          <div class="input-group">
            ...
            <input type="number" name="modal" class="form-control" min="0" required 
                   value="<?= $produk['modal'] ?>" id="modalInput">
          </div>
        ...
        <div>
          ...
          <div class="input-group">
            ...
            <input type="number" name="harga" class="form-control" min="0" required 
                   value="<?= $produk['harga'] ?>" id="hargaInput">
          </div>
      ...
      <div class="col-md-6">
        <div class="calculation-card">
          ...
          <div class="calculation-item">
            ...
            <strong id="modalDisplay">Rp <?= number_format($produk['modal']) ?></strong>
          </div>
          
          <div class="calculation-item">
            <span>Harga Jual:</span>
            <strong id="hargaDisplay">Rp <?= number_format($produk['harga']) ?></strong>
          </div>
          
          <div class="calculation-item">
            <span>Margin per Unit:</span>
            <strong class="text-info" id="marginDisplay">Rp <?= number_format($produk['margin']) ?></strong>
          </div>
          
          <div class="calculation-item">
            <span>Stok:</span>
            <strong id="stokDisplay"><?= $produk['stok'] ?> unit</strong>
          </div>
          
          <div class="calculation-item">
            <span>Total Keuntungan Potensial:</span>
            <strong class="text-success" id="keuntunganDisplay">Rp <?= number_format($produk['margin'] * $produk['stok']) ?></strong>
          </div>
        </div>
      </div>
    </div>
  ...
  <!-- INFORMASI TAMBAHAN -->
  <div class="col-md-12 form-section">
    <h5><i class="bi bi-clock-history me-2"></i> Informasi Tambahan</h5>
    
    <div class="row g-3">
      <div class="col-md-6">
        <div class="alert alert-light">
          <h6><i class="bi bi-calendar me-2"></i> Tanggal Pembuatan</h6>
          <p class="mb-0"><?= date('d F Y H:i', strtotime($produk['created_at'])) ?></p>
        </div>
      </div>
      <div class="col-md-6">
        <div class="alert alert-light">
          <h6><i class="bi bi-bar-chart me-2"></i> Statistik Keuntungan</h6>
          <p class="mb-0">
            Keuntungan per unit: <strong>Rp <?= number_format($produk['keuntungan']) ?></strong><br>
            Total keuntungan: <strong>Rp <?= number_format($produk['keuntungan'] * $produk['stok']) ?></strong>
          </p>
        </div>
      </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (harga < modal) {
    document.getElementById('hargaInput').classList.add('is-invalid');
    document.querySelector('button[type="submit"]').disabled = true;
  }
});
</script>
